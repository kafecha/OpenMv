...
# 西红柿 -> 5
# 辣椒 -> 6
color_map = {'qj':6, 'xhs':5, 'yc':3, 'ng':4, 'wcs':9}
# 使用代码：  color_code = color_map.get(color_name, None)
...
saoma_num = 0  #扫码次数
saoma_go = 0   #0-识别颜色  1-扫码  3-什么都不干  可以发送对应数字改变运行状态
        ...
        for threshold, color_name, frame_color in color_A_thresholds:
            ...
            for blob in blobs:
                if blob.pixels() >= area_threshold:
                    ...
                    min_length = length
                    color_code = color_map.get(color_name, None)
